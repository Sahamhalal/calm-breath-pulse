...
export function smoothnessLabel(value: number) {
  return (
    SMOOTHNESS_PRESETS.find((p) => value <= p.max)?.label ?? "Semula jadi"
  );
}

export function SmoothnessSetting({
  value,
  onChange,
}: {
  value: number;
        ...
        <div className="space-y-4">
          <div>
            <p className="text-sm font-medium text-foreground">
              Kelancaran peralihan nafas
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Melaraskan lengkung pergerakan antara tarik dan hembus nafas.
            </p>
          </div>
          <Slider
            value={[value]}
